...
const contentTypes = window.eZ.adminUiConfig.contentTypes;
const TEXT_SELECT_CONTENT_TYPE = Translator.trans(
    /*@Desc("Select a Content Type")*/ 'content_on_the_fly.select_a_content_type.title',
    {},
    'universal_discovery_widget'
);
const TEXT_TYPE_TO_REFINE = Translator.trans(
    /*@Desc("Type to refine")*/ 'content_on_the_fly.type_to_refine.placeholder',
    {},
    'universal_discovery_widget'
);

class ChooseContentTypeComponent extends Component {
    constructor(props) {
        super(props);
    ...
    findContentType(identifier) {
        ...
        Object.values(contentTypes).forEach((group) => {
    ...
    renderGroup(groupName, index) {
        const items = contentTypes[groupName];
        const groupAttrs = {};
    ...
    render() {
        ...
        return (
            <div className="c-choose-content-type">
                <p className="c-choose-content-type__title">{TEXT_SELECT_CONTENT_TYPE}</p>
                <div className="c-choose-content-type__list-wrapper">
                    <input className="form-control" type="text" placeholder={TEXT_TYPE_TO_REFINE} onChange={this.updateFilterQuery} />
...
ChooseContentTypeComponent.defaultProps = {
    preselectedContentType: null,
};

export default ChooseContentTypeComponent;
